...
//===----------------------------------------------------------------------===//
    ...
    for(auto& s : scopes) {
        out << "static inline _CCCL_DEVICE void __cuda_membar_" << s.first << "() { asm volatile(\"membar" << membar_scopes[s.first] << ";\":::\"memory\"); }\n";
        for(auto& sem : fence_semantics)
            out << "static inline _CCCL_DEVICE void " << fencename(sem.first, s.first) << "() { asm volatile(\"fence" << sem.second << s.second << ";\":::\"memory\"); }\n";
        ...
        for(auto& sz : ld_sizes) {
            for(auto& sem : ld_semantics) {
                ...
                out << "static inline _CCCL_DEVICE void __cuda_load_" << sem.first << "_" << sz << "_" << s.first << "(_CUDA_A __ptr, _CUDA_B& __dst) {";
                if(ld_as_atom)
        ...
        for(auto& sz : st_sizes) {
            for(auto& sem : st_semantics) {
                out << "template<class _CUDA_A, class _CUDA_B> ";
                out << "static inline _CCCL_DEVICE void __cuda_store_" << sem.first << "_" << sz << "_" << s.first << "(_CUDA_A __ptr, _CUDA_B __src) { ";
                ...
                for(auto& rmw: rmw_operations[cl]) {
                    for(auto& type: rmw_types[cl]) {
                        ...
                        for(auto& sem : rmw_semantics) {
                            ...
                            out << "static inline _CCCL_DEVICE void __cuda_" << rmw.first << "_" << sem.first << "_" << type.first << sz << "_" << s.first << "(";
                            if(rmw.first == "compare_exchange")
                        ...
                        for(auto& cv: cv_qualifier) {
                            if(rmw.first == "compare_exchange") {
                                out << "template<class _Type, _CUDA_VSTD::__enable_if_t<sizeof(_Type)==" << sz/8 << ", int> = 0>\n";
                                out << "_CCCL_DEVICE bool __atomic_compare_exchange_cuda(" << cv << "_Type *__ptr, _Type *__expected, const _Type *__desired, bool, int __success_memorder, int __failure_memorder, " << scopenametag(s.first) << ") {\n";
                                ...
                                if(rmw.first == "exchange") {
                                    out << ", int> = 0>\n";
                                    out << "_CCCL_DEVICE void __atomic_exchange_cuda(" << cv << "_Type *__ptr, _Type *__val, _Type *__ret, int __memorder, " << scopenametag(s.first) << ") {\n";
                                    out << "    uint" << sz << "_t __tmp = 0;\n";
                                    out << "    memcpy(&__tmp, __val, " << sz/8 << ");\n";
                                }
                                else {
                                    if(type.first == "f")
                                        ...
                                        out << ", int> = 0>\n";
                                    out << "_CCCL_DEVICE _Type __atomic_" << rmw.first << "_cuda(" << cv << "_Type *__ptr, _Type __val, int __memorder, " << scopenametag(s.first) << ") {\n";
                                    out << "    _Type __ret;\n";
