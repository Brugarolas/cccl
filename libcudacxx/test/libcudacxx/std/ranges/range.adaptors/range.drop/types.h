...
public:
  ...
  __host__ __device__ friend constexpr bool operator!=(drop_sentinel const s, T* const ptr) noexcept { return !(s == ptr); }
  __host__ __device__ friend constexpr bool operator!=(T* const ptr, drop_sentinel const s) noexcept { return !(s == ptr); }
};
...
struct MaybeSimpleNonCommonView : cuda::std::ranges::view_base {
  int start_;
  int* num_of_sentinel_cmp_calls;
  __host__ __device__ constexpr cuda::std::size_t size() const { return 8; }
  __host__ __device__ constexpr int* begin() { return globalBuff + start_; }
  __host__ __device__ constexpr cuda::std::conditional_t<IsSimple, int*, const int*> begin() const { return globalBuff + start_; }
  __host__ __device__ constexpr drop_sentinel<int> end() { return drop_sentinel<int>{globalBuff + size(), num_of_sentinel_cmp_calls}; }
