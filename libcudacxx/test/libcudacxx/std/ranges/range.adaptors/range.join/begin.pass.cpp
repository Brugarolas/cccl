...
template <class T>
concept HasConstBegin = requires(const T& t) {
  t.begin();
};
...
template <class T>
inline constexpr bool HasConstBegin<
    T, cuda::std::void_t<decltype(cuda::std::declval<const T&>().begin())> > =
    true;
#endif // TEST_STD_VER <= 2017

__host__ __device__ TEST_CONSTEXPR_CXX20 bool test() {
  int buffer[4][4] = {{1111, 2222, 3333, 4444},
                      {555, 666, 777, 888},
                      {99, 1010, 1111, 1212},
                      {13, 14, 15, 16}};
...

  {
    CopyableChild children[4] = {
        CopyableChild(buffer[0], 4), CopyableChild(buffer[1], 0),
        CopyableChild(buffer[2], 1), CopyableChild(buffer[3], 0)};
    auto jv = cuda::std::ranges::join_view(ParentView{children});
    assert(*jv.begin() == 1111);
  }
  ...
  {
    CopyableChild children[4] = {
        CopyableChild(buffer[0]), CopyableChild(buffer[1]),
        CopyableChild(buffer[2]), CopyableChild(buffer[3])};
  ...
  {
    CopyableChild children[4] = {
        CopyableChild(buffer[0], 0), CopyableChild(buffer[1], 0),
        CopyableChild(buffer[2], 0), CopyableChild(buffer[3], 0)};
    auto jv = cuda::std::ranges::join_view(ParentView{children});
    assert(jv.begin() == jv.end());
  }
  ...
  {
    CopyableChild children[4] = {
        CopyableChild(buffer[0], 4), CopyableChild(buffer[1], 4),
        CopyableChild(buffer[2], 4), CopyableChild(buffer[3], 0)};
    auto jv = cuda::std::ranges::join_view(ParentView{children});
    assert(*jv.begin() == 1111);
  }
  ...
  // !is_reference_v<range_reference_t<const V>>
  {
    auto innerRValueRange =
        cuda::std::views::iota(0, 5) |
        cuda::std::views::transform([](int) { return ChildView{}; });
    static_assert(
        !cuda::std::is_reference_v<cuda::std::ranges::range_reference_t<
            const decltype(innerRValueRange)> >);
  ...
  // !simple-view<V>
  {
    ...
    static_assert(
        !cuda::std::same_as<decltype(jv.begin()),
                            decltype(cuda::std::as_const(jv).begin())>);
    unused(jv);
  }
  ...
  // Check stashing iterators (LWG3698: regex_iterator and join_view don't work together very well)
  {
    ...
    assert(cuda::std::ranges::equal(cuda::std::views::counted(jv.begin(), 10),
                                    cuda::std::string_view{"aababcabcd"}));
  }
