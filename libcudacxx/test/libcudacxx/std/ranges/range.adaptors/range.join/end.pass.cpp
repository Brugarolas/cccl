...
template <class T>
inline constexpr bool HasConstEnd<
    T, cuda::std::void_t<decltype(cuda::std::declval<const T&>().end())> > =
    true;
...
__host__ __device__ TEST_CONSTEXPR_CXX20 bool test() {
...

  {
    ...
    assert(cuda::std::as_const(jv).end() ==
           cuda::std::ranges::next(cuda::std::as_const(jv).begin(), 16));

    static_assert(HasConstEnd<decltype(jv)>);
    static_assert(cuda::std::same_as<decltype(jv.end()),
                                     decltype(cuda::std::as_const(jv).end())>);
...

  {
    ...
    assert(cuda::std::as_const(jv).end() ==
           cuda::std::ranges::next(cuda::std::as_const(jv).begin(), 12));
...

  {
    ...
    assert(cuda::std::as_const(jv).end() ==
           cuda::std::ranges::next(cuda::std::as_const(jv).begin(), 4));
