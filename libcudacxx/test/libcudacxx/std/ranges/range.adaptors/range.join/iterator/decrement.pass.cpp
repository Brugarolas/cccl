...
template <class T>
concept CanPreDecrement = requires(T & t) {
  --t;
};

template <class T>
concept CanPostDecrement = requires(T & t) {
  t--;
};
...
template <class T>
inline constexpr bool CanPreDecrement<
    T, cuda::std::void_t<decltype(--cuda::std::declval<T&>())> > = true;

template <class T, class = void>
inline constexpr bool CanPostDecrement = false;

template <class T>
inline constexpr bool CanPostDecrement<
    T, cuda::std::void_t<decltype(cuda::std::declval<T&>()--)> > = true;
...
__host__ __device__ TEST_CONSTEXPR_CXX20 bool test() {
...
#endif
  {
    ...
    BidiCommonInner inners[4] = {
        buffer[0], {nullptr, 0}, {nullptr, 0}, buffer[1]};
