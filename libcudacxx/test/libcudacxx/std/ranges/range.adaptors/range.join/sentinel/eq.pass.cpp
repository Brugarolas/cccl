...
inline constexpr bool EqualityComparable =
    cuda::std::invocable<cuda::std::equal_to<>, const Iter&, const Sent&>;
...
struct ConstComparableSentinel {
  ...
  operator==(const Iterator& i, const ConstComparableSentinel& s) {
    return base(i) == base(s.iter_);
  }
#if TEST_STD_VER <= 2017
  __host__ __device__ constexpr friend bool
  operator==(const ConstComparableSentinel& s, const Iterator& i) {
    return base(i) == base(s.iter_);
  }
  __host__ __device__ constexpr friend bool
  operator!=(const Iterator& i, const ConstComparableSentinel& s) {
  ...
  __host__ __device__ constexpr friend bool
  operator==(const ConstIterator& i, const ConstComparableSentinel& s) {
    return base(i) == base(s.iter_);
  }
#if TEST_STD_VER <= 2017
  __host__ __device__ constexpr friend bool
  operator==(const ConstComparableSentinel& s, const ConstIterator& i) {
    return base(i) == base(s.iter_);
  }
  __host__ __device__ constexpr friend bool
  operator!=(const ConstIterator& i, const ConstComparableSentinel& s) {
...
struct ConstComparableView : BufferView<BufferView<int*>*> {
  ...
  template <class T>
  __host__ __device__ constexpr ConstComparableView(T&& arr)
...
static_assert(EqualityComparable<
              cuda::std::ranges::iterator_t<ConstComparableView>,
              cuda::std::ranges::sentinel_t<const ConstComparableView> >);
static_assert(
    EqualityComparable<cuda::std::ranges::iterator_t<const ConstComparableView>,
...
__host__ __device__ constexpr bool test() {
  ...
  // test iterator<true> == sentinel<true>
  {
    ...
    using ParentT = cuda::std::remove_all_extents_t<decltype(children)>;
    const auto jv =
        cuda::std::ranges::join_view(ForwardParentView<ParentT>(children));
    assert(jv.end() == cuda::std::ranges::next(jv.begin(), 16));
  }
