...
struct ParentView : cuda::std::ranges::view_base {
  ...
  __host__ __device__ constexpr ParentView(T* ptr, unsigned size = 4)
      : ptr_(ptr), size_(size) {}
  template <class T2 = T,
            cuda::std::enable_if_t<cuda::std::same_as<ChildView, T2>, int> = 0>
  __host__ __device__ constexpr ParentView(ChildView* ptr = globalChildren,
                                           unsigned size = 4)
      : ptr_(ptr), size_(size) {}
...
struct CopyableChild : cuda::std::ranges::view_base {
  ...
  __host__ __device__ constexpr CopyableChild(int* ptr = globalBuffer[0],
                                              unsigned size = 4)
      : ptr_(ptr), size_(size) {}
...
struct InputValueIter {
  ...
  operator==(const InputValueIter&, const InputValueIter&) = default;
#else  // ^^^ C++20 ^^^ / vvv C++17 vvv
  __host__ __device__ constexpr friend bool
  operator==(const InputValueIter& lhs, const InputValueIter& rhs) {
...
struct BufferView : cuda::std::ranges::view_base {
  ...
  template <class Iter2 = Iter,
            cuda::std::enable_if_t<!cuda::std::is_same_v<Iter2, NonConstIter>,
                                   int> = 0>
  ...
  template <class Sent2 = Sent,
            cuda::std::enable_if_t<!cuda::std::is_same_v<Sent2, NonConstSent>,
                                   int> = 0>
...
using InputNonCommonInner =
    BufferView<common_input_iterator<int*>,
...
using ForwardNonCommonInner =
    BufferView<forward_iterator<int*>,
...
using BidiNonCommonInner =
    BufferView<bidirectional_iterator<int*>,
...
static_assert(
    !cuda::std::ranges::bidirectional_range<SimpleInputCommonOuter<> >);
...
static_assert(
    !cuda::std::ranges::bidirectional_range<NonSimpleInputCommonOuter<> >);
...
static_assert(
    !cuda::std::ranges::bidirectional_range<SimpleForwardCommonOuter<> >);
...
using NonSimpleForwardCommonOuter =
    BufferView<forward_iterator<const Inner*>, forward_iterator<const Inner*>,
               forward_iterator<Inner*>, forward_iterator<Inner*> >;
static_assert(cuda::std::ranges::forward_range<NonSimpleForwardCommonOuter<> >);
static_assert(
    !cuda::std::ranges::bidirectional_range<NonSimpleForwardCommonOuter<> >);
...
template <class Inner = BufferView<int*> >
using SimpleForwardNonCommonOuter =
    BufferView<forward_iterator<Inner*>,
               sentinel_wrapper<forward_iterator<Inner*> > >;
static_assert(cuda::std::ranges::forward_range<SimpleForwardNonCommonOuter<> >);
static_assert(
    !cuda::std::ranges::bidirectional_range<SimpleForwardNonCommonOuter<> >);
...
using NonSimpleForwardNonCommonOuter =
    BufferView<forward_iterator<const Inner*>,
               ...
               sentinel_wrapper<forward_iterator<Inner*> > >;
static_assert(
    cuda::std::ranges::forward_range<NonSimpleForwardNonCommonOuter<> >);
static_assert(
    !cuda::std::ranges::bidirectional_range<NonSimpleForwardNonCommonOuter<> >);
static_assert(
    !cuda::std::ranges::common_range<NonSimpleForwardNonCommonOuter<> >);
...
struct copying_iterator {
  ...
  __host__ __device__ constexpr copying_iterator()
      noexcept(cuda::std::is_nothrow_default_constructible_v<It2>) {}
  ...
  operator==(copying_iterator const&, copying_iterator const&) = default;
#else  // ^^^ C++20 ^^^ / vvv C++17 vvv
  __host__ __device__ constexpr friend bool
  operator==(const copying_iterator& lhs, const copying_iterator& rhs) {
...
struct InnerRValue : Outer {
  ...
  using const_iterator =
      copying_iterator<cuda::std::ranges::iterator_t<const Outer> >;
  using sentinel = copying_iterator<cuda::std::ranges::sentinel_t<Outer> >;
  using const_sentinel =
      copying_iterator<cuda::std::ranges::sentinel_t<const Outer> >;

#if defined(TEST_COMPILER_NVRTC)
  constexpr InnerRValue() noexcept = default;

  template <class... _Args>
  __host__ __device__ constexpr InnerRValue(_Args&&... __args)
  ...
  static_assert(cuda::std::ranges::common_range<Outer>,
                "non-common range is not supported yet");

  __host__ __device__ constexpr iterator begin() { return Outer::begin(); }
  template <
      class Outer2 = Outer,
      cuda::std::enable_if_t<cuda::std::ranges::range<const Outer2>, int> = 0>
  __host__ __device__ constexpr const_iterator begin() const {
    return Outer::begin();
  }
...
static_assert(cuda::std::ranges::forward_range<
              InnerRValue<SimpleForwardCommonOuter<> > >);
static_assert(!cuda::std::ranges::bidirectional_range<
              InnerRValue<SimpleForwardCommonOuter<> > >);
static_assert(
    cuda::std::ranges::common_range<InnerRValue<SimpleForwardCommonOuter<> > >);
static_assert(simple_view<InnerRValue<SimpleForwardCommonOuter<> > >);
static_assert(
    !cuda::std::is_lvalue_reference_v<cuda::std::ranges::range_reference_t<
        InnerRValue<SimpleForwardCommonOuter<> > > >);
...
struct move_swap_aware_iter {
  ...
  __host__ __device__ constexpr friend bool
  operator==(const move_swap_aware_iter& x, const move_swap_aware_iter& y) {
    return x.i_ == y.i_;
  }
  ...
  __host__ __device__ constexpr friend rvalue_reference
  iter_move(const move_swap_aware_iter& x) noexcept {
    ++(*x.iter_move_called);
    return rvalue_reference{cuda::std::move(*x.i_), cuda::std::move(*x.i_)};
  }

  __host__ __device__ TEST_CONSTEXPR_CXX20 friend void
  iter_swap(const move_swap_aware_iter& x,
            const move_swap_aware_iter& y) noexcept {
    ++(*x.iter_swap_called);
...
struct IterMoveSwapAwareView : BufferView<int*> {
  ...
  template <class T>
  __host__ __device__ constexpr IterMoveSwapAwareView(T&& arr)
  ...
  __host__ __device__ constexpr auto end() {
    return move_swap_aware_iter{&iter_move_called, &iter_swap_called,
                                data_ + size_};
  }
};
...
public:
  ...
  __host__ __device__ constexpr StashingIterator() : letter_('a') {}
  ...
  __host__ __device__ constexpr value_type operator*() const { return str_; }

  __host__ __device__ constexpr bool
  operator==(cuda::std::default_sentinel_t) const {
    return letter_ > 'z';
  }
#if TEST_STD_VER <= 2017
  __host__ __device__ constexpr bool
...
using StashingRange =
    cuda::std::ranges::subrange<StashingIterator,
...
public:
  __host__ __device__ constexpr StashingIterator begin() { return {}; }
  __host__ __device__ constexpr cuda::std::default_sentinel_t end() {
    return {};
  }

  __host__ __device__ constexpr const int* begin() const { return &val_; }
  __host__ __device__ constexpr const int* end() const { return &val_ + 1; }
...
static_assert(cuda::std::ranges::input_range<
              cuda::std::ranges::range_reference_t<ConstNonJoinableRange> >);
static_assert(
    !cuda::std::ranges::input_range<
        cuda::std::ranges::range_reference_t<const ConstNonJoinableRange> >);
